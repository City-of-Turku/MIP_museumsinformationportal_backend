...
class PublicValintaController extends Controller {
		...
		try {
			if ($type == 'rakennuskiinteisto') {
				...
				$types = [
					...
					$result = [
							"categoryName" => "Rakennukset ja kiinteistöt",
					...
					foreach ($types as $key => $entityClass) {
							...
							foreach ($entities as $entity) {
									$values[] = [
											"value" => $entity->id,
											"nimi_fi" => $entity->nimi_fi,
											"nimi_se" => $entity->nimi_se,
											"nimi_en" => $entity->nimi_en
									];
							}
