...
class MMLQueries {
	...
	public static function getKiinteistoTiedot_byHttpGET($bbox) {
		...
		Log::channel('mml')->info('FUNCTION: MMLQueries.getKiinteistoTiedot_byHttpGET(), USER: ' . Auth::user()->id .', BBOX: ' . $bbox);

		if ($res->getStatusCode()!="200") {
			Log::channel('mml')->info('    REQUEST FAILED');
			throw new Exception("KiinteistoTiedot failed: ".$res->getStatusCode()." : ".$res->getReasonPhrase());
		}

		// Log::channel('mml')->info('    RESPONSE: ' . $res->getBody());
	...
	public static function getKiinteistoTunnusByPoint($point) {
		...
		Log::channel('mml')->info('FUNCTION: MMLQueries.getKiinteistoTunnusByPoint(), USER: ' . Auth::user()->id .', POINT: ' . $point);
	...
	public static function getKiinteistoTunnusByPolygon($polygon) {
		...
		Log::channel('mml')->info('FUNCTION: MMLQueries.getKiinteistoTunnusByPolygon(), USER: ' . Auth::user()->id .', POLYGON: ' . $polygon);
	...
	public static function getKiinteistoTiedotREST($kiinteistotunnus) {
		...
		Log::channel('mml')->info('FUNCTION: MMLQueries.getKiinteistoTiedotREST(), USER: ' . Auth::user()->id .', KIINTEISTOTUNNUS: ' . $kiinteistotunnus);
	...
	public static function getRakennusTiedot($kiinteistotunnus, $sijainti) {
		...
		Log::channel('mml')->info('FUNCTION: MMLQueries.getRakennusTiedot(), USER: ' . Auth::user()->id .', KIINTEISTOTUNNUS: ' . $kiinteistotunnus . ', SIJAINTI: ' . $sijainti);
	...
	private static function parseOsoiteTiedot($rawXmlString) {

		$xml_string = str_replace(array("gml:","wfs:", "oso:"), "", $rawXmlString);
		...
		if($data->featureMember) {
			...
			foreach($data->featureMember as $osoite) {
				$result = array(); //Osoite

				$result['katunimi'] = (string)$osoite->Osoitenimi->katunimi;
				$result['katunumero'] = (string)$osoite->Osoitenimi->katunumero;
				$result['postinumero'] = (string)$osoite->Osoitenimi->postinumero;

				$result['kieli'] = (string)$osoite->Osoitenimi->kieli;
				$result['jarjestysnumero'] = (string)$osoite->Osoitenimi->jarjestysnumero;

				$result['kuntatunnus'] = (string)$osoite->Osoitenimi->kuntatunnus;
				$result['kuntanimiFin'] = (string)$osoite->Osoitenimi->kuntanimiFin;
				$result['kuntanimiSwe'] = (string)$osoite->Osoitenimi->kuntanimiSwe;

				$result['sijainti'] = (string)$osoite->Osoitenimi->sijainti->Point->pos;
	...
	public static function getOsoiteTiedot($katunimi, $kuntanimi, $kuntanumero) {
		...
		$url = config('app.mml_maasto_url');
		$username = config('app.mml_wmts_username');
		$password = config('app.mml_wmts_password');
		...
		$res = $client->request("POST", $url, [
				...
				'headers' => ['Content-Type' => 'text/xml'],
				"auth" => [$username, $password],
				"body" => self::generateOsoiteTiedotRequestBody($katunimi, $kuntanimi, $kuntanumero)
		]);

		Log::channel('mml')->info('FUNCTION: MMLQueries.getOsoiteTiedot(), USER: ' . Auth::user()->id .', KATUNIMI: ' . $katunimi . ', KUNTANIMI: ' . $kuntanimi . ', KUNTANUMERO: ' . $kuntanumero);
	...
	private static function parseNimistoTiedot($rawXmlString) {
		...
		if($data->featureMember) {
			...
			$kunnatXml = MMLNimiMapper::getCodingsXml('http://xml.nls.fi/Nimisto/Nimistorekisteri/kunta.xsd');
			$paikkatyypitXml = MMLNimiMapper::getCodingsXml('http://xml.nls.fi/Nimisto/Nimistorekisteri/paikkatyyppi.xsd');
			...
			foreach($data->featureMember as $paikka) {
				...
				//Map kunta etc
				$result['kunta'] = MMLNimiMapper::mapKuntaValue($paikka->Paikka->kuntaKoodi, $kunnatXml);
				$result['paikkatyyppi'] = MMLNimiMapper::mapPaikkatyyppiValue($paikka->Paikka->paikkatyyppiKoodi, $paikkatyypitXml);

				foreach($paikka->Paikka->nimi as $paikannimi) {
					if($result['nimi'] != "") {
						$result['nimi'] = $result['nimi'] . ", " . (string)$paikannimi->PaikanNimi->kirjoitusasu; //Retuns all of the names separated with comma. E.g. Turku, Åbo
					} else {
						$result['nimi'] = (string)$paikannimi->PaikanNimi->kirjoitusasu;
					}
				}

				$result['formatted_data'] = (string)$result['nimi'] . " (" . $result['paikkatyyppi'] . "), " . $result['kunta'];

				$result['sijainti'] = (string)$paikka->Paikka->paikkaSijainti->Point->pos;
	...
	public static function getNimistoTiedot($paikannimi, $kunta, $kuntahaku) {
		...
		$url = config('app.mml_nimisto_url');
		$username = config('app.mml_wmts_username');
		$password = config('app.mml_wmts_password');
		...
		if($kuntahaku) { //Haetaan ainoastaan kunta ja sen sijainti. Paikannimi kentässä on tällöin kuntanumero ja kuntahaku on true
			$res = $client->request("POST", $url, [
					...
					'headers' => ['Content-Type' => 'text/xml'],
					"auth" => [$username, $password],
					"body" => self::generateKuntahakuRequestBody($paikannimi)
			]);
		} else {
			$res = $client->request("POST", $url, [
					'http_errors' => false,
					'headers' => ['Content-Type' => 'text/xml'],
					"auth" => [$username, $password],
					"body" => self::generateNimistoTiedotRequestBody($paikannimi, $kunta)
			]);
		}

		Log::channel('mml')->info('FUNCTION: MMLQueries.getNimistoTiedot(), USER: ' . Auth::user()->id .', PAIKANNIMI: ' . $paikannimi . ', KUNTA: ' . $kunta . ', KUNTAHAKU: ' . $kuntahaku);
