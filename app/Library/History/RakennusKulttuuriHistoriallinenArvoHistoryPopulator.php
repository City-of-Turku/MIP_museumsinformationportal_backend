...
class RakennusKulttuuriHistoriallinenArvoHistoryPopulator {
	...
	public function populate($hi_entry) {
		...
		if (array_key_exists("kulttuurihistoriallinenarvo_id", $row_data)) {
			$row_data["kulttuurihistoriallinenarvo"] = RakennuksenKulttuurihistoriallinenArvo::getSingle($row_data["kulttuurihistoriallinenarvo_id"])->withTrashed()->first();
		}
		
		if (!is_null($changed_fields) && array_key_exists("kulttuurihistoriallinenarvo_id", $changed_fields)) {
			$changed_fields["kulttuurihistoriallinenarvo"] = RakennuksenKulttuurihistoriallinenArvo::getSingle($changed_fields["kulttuurihistoriallinenarvo_id"])->withTrashed()->first();
		}
